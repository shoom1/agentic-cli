"""Resolution + asset tests for the demo report-writer skill (no LLM)."""
...
def test_skill_resolves():
    skills = SkillStore([str(_SKILLS_DIR)]).resolve(["report-writer"])
    assert len(skills) == 1
    assert skills[0].name == "report-writer"
    assert skills[0].description  # non-empty when-to-use


def test_template_asset_present():
    tpl = _SKILLS_DIR / "report-writer" / "assets" / "report_template.tex"
    assert tpl.is_file()
    body = tpl.read_text()
    assert "\\documentclass" in body and "\\includegraphics" in body


def test_toolset_excludes_scripts():
    skills = SkillStore([str(_SKILLS_DIR)]).resolve(["report-writer"])
